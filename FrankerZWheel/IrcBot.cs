    ...
    public abstract class IrcBot
    {
        ...
        private void RecieveComplete(IAsyncResult result)
        {
            ...
            foreach (var message in messages)
            {
                ...
                {
                    Console.Error.WriteLine("shit's broken: " + ex.Message);
                }
            }
        ...
        private void HandleMessage(IrcMessage message)
        {
            ...
            switch (message.Command)
            {
                ...
                case "PART":
                    ...
                    {
                        Console.Error.WriteLine("Could not confirm PART... but it probably worked");
                    }
                    break;
                case "JOIN":
                    channelName = message.Parameters[0].TrimStart(':');
                    channel = GetChannel(channelName);
        ...
        {
            return Channels.Where(c => c.Name == name).First();
        }
